 ...
 */
public class GameRuleGomokuStandard implements GameRule {
	...
	protected final boolean winExactly_ = true;
	...
	public GamePiece evalRow ( List<Move> sequence, GameState state ) {
		...
		for ( int i = 0 ; i < sequence.size() ; i++ ) {
			if ( sequence.get(i).piece_ == GamePiece.NONE ) { // reset tracker
				...
				runCount++;
				if (gotWinCount && runCount == winCount_) {	// exceed winCount_, no win
					gotWinCount = false;
					winPiece = GamePiece.NONE;
				}
		...
		if (gotWinCount) {
			return winPiece; // winCount_ gamePieces in a row, return winner
		}
